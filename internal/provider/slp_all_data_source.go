...
var (
	_ datasource.DataSource              = &slpAllDataSource{}
	_ datasource.DataSourceWithConfigure = &slpAllDataSource{}
)

// slpAllDataSource is the data source implementation.
type slpAllDataSource struct {
	client  *backupdr.APIClient
	authCtx context.Context
}

// tf go model
type allSlpsResourceModel struct {
	// Count types.Int64        `tfsdk:"count"`
	Items []SlpResourceModel `tfsdk:"items"`
}

// NewSlpAllDataSource - Datasource for SLA Profile
func NewSlpAllDataSource() datasource.DataSource {
	return &slpAllDataSource{}
}

// Configure adds the provider configured client to the data source.
func (d *slpAllDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *slpAllDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_slp_all"
}

func (d *slpAllDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
...
func (d *slpAllDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {

	var state allSlpsResourceModel
	...
	// state.Count = types.Int64Value(int64(slp.Count))
	state.Items = slps
